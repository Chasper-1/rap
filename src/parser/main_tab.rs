...
pub fn draw_main_layout(f: &mut Frame, area: Rect) {
    ...
    f.render_widget(Block::default().borders(Borders::TOP | Borders::LEFT | Borders::RIGHT), top_parts[0]);
    ...
    f.render_widget(Block::default().borders(Borders::LEFT | Borders::RIGHT), stepped_area);
    ...
    if conf.ui.step_offset > 0 {
        ...
        buf.set_string(x_left_search + 1, y_join, &line, ratatui::style::Style::default());
        
        // Углы ступеньки
        if let Some(cell) = buf.cell_mut((x_left_search, y_join)) { cell.set_symbol("└"); }
        if let Some(cell) = buf.cell_mut((x_left_main, y_join)) { cell.set_symbol("┐"); }
    }
